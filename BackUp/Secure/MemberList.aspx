				...
				<div class="headInfo admin-content-bg-white">
					<portal:HeadingPanel ID="heading" runat="server">
						...
						<asp:Button SkinID="ExportButton" ID="btnExport" OnClick="btnExport_Click"
							Visible='<%# canManageUsers %>' Text="<%$Resources:Resource, ExportDataAsExcel %>"
					...
					<div class="form-horizontal">
						<asp:Panel ID="divNewUser" CssClass="row" runat="server" DefaultButton="btnSearchUser">
							...
							<div id="divIPLookup" runat="server" visible="false" class="col-12 mt-15">
								<div class="input-group">
						...
						<asp:Panel ID="pnlLocked" runat="server" Cssclass="my-15">
						...
						<div class="mrb10">
							<div id="spnTopPager" class='table-paginate' runat="server"></div>
						</div>
					</div>
				</div>
			</div>
					...
					<telerik:RadGrid ID="grid" SkinID="radGridSkin" runat="server">
						<MasterTableView DataKeyNames="UserID" AllowPaging="false">
							<Columns>
								...
								<telerik:GridTemplateColumn HeaderText="<%$Resources:Resource, Name %>"
									DataField="Name">
									<ItemTemplate>
										<strong>
										...
										<div id="Div1" runat="server" visible='<%# ShowEmailInMemberList %>'>
											&nbsp;<a
												href='<%# "mailto:" + Server.HtmlEncode(DataBinder.Eval(Container.DataItem, "Email").ToString())%>'><%# Server.HtmlEncode(DataBinder.Eval(Container.DataItem, "Email").ToString())%></a>
										</div>
										...
										<div id="Div3" runat="server" visible='<%# ShowUserIDInMemberList %>'>
											&nbsp;
											<gb:SiteLabel ID="lblTotalPosts" runat="server"
												ConfigKey="RegisterLoginNameLabel" UseLabelTag="false" />
											<%# Server.HtmlEncode(DataBinder.Eval(Container.DataItem, "UserID").ToString())%>
										</div>
										<div id="Div4" runat="server"
											visible='<%# (IsAdmin && (!Convert.ToBoolean(Eval("DisplayInMemberList")))) %>'
								...
								<telerik:GridTemplateColumn HeaderStyle-Width="150"
									HeaderText="<%$Resources:Resource, MemberListDateCreatedLabel %>"
								...
								<telerik:GridTemplateColumn
									HeaderText="<%$Resources:Resource, MemberListUserWebSiteLabel %>"
									DataField="WebSiteUrl">
									<ItemTemplate>
										<a rel="nofollow"
											href='<%# Server.HtmlEncode(DataBinder.Eval(Container.DataItem,"WebSiteUrl").ToString()) %>'>
								...
								<telerik:GridTemplateColumn HeaderStyle-Width="200" AllowFiltering="false">
									<ItemTemplate>
										<a class="popup-link cp-link"
											href='<%# SiteRoot + "/Secure/ProfileView.aspx?userid=" + DataBinder.Eval(Container.DataItem,"UserID") %>'>
											<gb:SiteLabel ID="lblViewProfile" runat="server"
												ConfigKey="MemberListViewProfileLabel" UseLabelTag="false" /></a>
										<asp:HyperLink CssClass="cp-link"
											Text='<%# Resources.Resource.ManageUserLink %>' ID="Hyperlink2"
