				...
				<MasterTableView DataKeyNames="PageID" EditMode="InPlace" AllowPaging="false" AllowSorting="false">
					<Columns>
						...
						<telerik:GridTemplateColumn ItemStyle-HorizontalAlign="center">
							...
							<EditItemTemplate>
								<asp:CheckBox ID="chkPublished" runat="server"
									Checked='<%# WebUtils.NullToFalse(Eval("IsPublished")) %>' />
								<asp:Button SkinID="UpdateButton" ID="btnGridUpdate" runat="server"
									Text='<%# GetUpdateButtonText() %>' CommandName="Update"
									EnableViewState='<%# !useDialogForEditing %>' />
								<asp:Button SkinID="CancelButton" ID="btnGridCancel" runat="server"
									Text='<%# GetCancelButtonText() %>' CommandName="Cancel"
									EnableViewState='<%# !useDialogForEditing %>' />
							</EditItemTemplate>
						</telerik:GridTemplateColumn>
						<telerik:GridTemplateColumn ItemStyle-HorizontalAlign="center">
							...
							<EditItemTemplate>
								<asp:DropDownList ID="ddPaneNames" runat="server" DataTextField="key"
									DataValueField="value" EnableViewState='<%# !useDialogForEditing %>'
						...
						<telerik:GridTemplateColumn ItemStyle-HorizontalAlign="center">
							...
							<EditItemTemplate>
								<asp:TextBox ID="txtModuleOrder" Columns="4"
									Text='<%# GetModuleOrder(Eval("ModuleOrder")) %>' runat="server"
									EnableViewState='<%# !useDialogForEditing %>' />
							</EditItemTemplate>
						</telerik:GridTemplateColumn>
						<telerik:GridTemplateColumn>
							...
							<EditItemTemplate>
								<gb:DatePickerControl ID="dpBeginDate" runat="server"
									Text='<%# GetBeginDate(Eval("PublishBeginDate")) %>' ShowTime="True"
									SkinID="ContentManager" EnableViewState='<%# !useDialogForEditing %>'>
								</gb:DatePickerControl>
								<asp:RequiredFieldValidator ID="reqElement" runat="server"
						...
						<telerik:GridTemplateColumn>
							...
							<EditItemTemplate>
								<gb:DatePickerControl ID="dpEndDate" runat="server"
									Text='<%# GetEndDate(Eval("PublishEndDate")) %>' ShowTime="True"
									SkinID="ContentManager" EnableViewState='<%# !useDialogForEditing %>'>
								</gb:DatePickerControl>
							</EditItemTemplate>
						</telerik:GridTemplateColumn>
